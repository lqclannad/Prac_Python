...
# 创建训练器
class Trainer:
    def __init__(self, net, save_path, last_path, dataset_path, isCuda=True):  # 网络，参数保存路径，训练数据路径，cuda加速为True
        self.net = net
        ...
        # 恢复网络训练---加载模型参数，继续训练
        if os.path.exists(self.save_path):  # 如果文件存在，接
            # 着继续训练
    ...
    def train(self):
        ...
        dataloader = DataLoader(faceDataset, batch_size=1500, shuffle=True, num_workers=0, drop_last=False)  # 数据加载器
        ...
        while True:
            ...
            for i, (img_data_, category_, offset_) in enumerate(dataloader):  # 样本，置信度，偏移量
                ...
                del loss, cls_loss, offset_loss
            # 保存最后的
            torch.save(self.net.state_dict(), self.last_path)
            # 保存最好的
            avg_loss = sum_loss / len(dataloader)
            if avg_loss < min_loss:
                min_loss = avg_loss
                torch.save(self.net.state_dict(), self.save_path)  # state_dict保存网络参数，save_path参数保存路径
                pro_net = self.save_path.split("/")[1].split(".")[0]
                with open(f"param/{pro_net}_log.txt", "a") as f:
                    f.write(f"{pro_net} {epoch} min_avg_loss {min_loss}\n")
                print(f"轮次{epoch} {pro_net}模型 最小损失{min_loss} save success")  # 每轮次保存一次；最好做一判断：损失下降时保存一次
