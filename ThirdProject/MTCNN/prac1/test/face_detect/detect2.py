...
class Detector:
    ...
    def detect(self, image): # 检测图片
        ...
        print("p.shape:",pnet_boxes.shape)
        ...
        print("r.shape:",rnet_boxes.shape)
        ...
        print("o.shape:",onet_boxes.shape)
    ...
    def __pnet_detect(self, image): # ★p网络全部是卷积，与输入图片大小无关，可输出任意形状图片
        ...
        while min_side_len > 12: # 直到缩放到小于等于12时停止
            ...
            # img = cv2.resize(np.array(img), (_w, _h), interpolation=cv2.INTER_CUBIC)
    ...
    def __rnet_detect(self, image, pnet_boxes):
        ...
        for _box in _pnet_boxes: # ★遍历每个框，每个框返回框4个坐标点，抠图，放缩，数据类型转换，添加列表
            ...
            # img = cv2.resize(np.array(img), (24, 24), interpolation=cv2.INTER_CUBIC)
            # img = PIL.Image.fromarray(img)
            img_data = self.__image_transform(img) # 将图片数组转成张量
    ...
    def __onet_detect(self, image, rnet_boxes):
        ...
        for _box in _rnet_boxes: # 遍历R网络筛选出来的框，计算坐标，抠图，缩放，数据类型转换，添加列表，堆叠
            ...
            # img = cv2.resize(np.array(img), (48, 48), interpolation=cv2.INTER_CUBIC)
            # img = PIL.Image.fromarray(img)
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if ret:
            ...
            # frame = cv2.convertScaleAbs(frame,alpha=2,beta=2)
