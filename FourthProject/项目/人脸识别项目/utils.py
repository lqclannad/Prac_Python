...
def compare(face_feature, min_cosa):
    face1_norm = F.normalize(face_feature)
    pass_faces = {}
    all_pass_count = 0
    for registed_face in os.listdir("register"):
        face_features = torch.load(f"register/{registed_face}")
        registed_face = registed_face[:-3]
        pass_count = 0
        for i,face_feature2 in enumerate(face_features):
            face2_norm = F.normalize(face_feature2)
            cosa = torch.matmul(face1_norm, face2_norm.t())
            if cosa > min_cosa:
                print(registed_face,cosa)
                pass_count += 1
                all_pass_count += 1
        pass_faces[registed_face] = pass_count
    max_rate = 0
    max_key = "陌生人"
    if all_pass_count > 0:
        for key in pass_faces.keys():
            pass_count = pass_faces[key]
            pass_rate = pass_count / all_pass_count
            if pass_rate > max_rate:
                max_rate = pass_rate
                max_key = key
            pass_faces[key] = (pass_count, pass_rate)
        print("--------", pass_faces)
    return max_key, max_rate
